...
$sa_id = intval($_SESSION['user_id'] ?? 0);

// ── Superadmin Profile ────────────────────────────────────────────────────────
$sa_stmt = $conn->prepare("SELECT user_id, firstname, lastname, username, email, role, profile_picture_url, created_at FROM users WHERE user_id = ?");
$sa_stmt->bind_param("i", $sa_id);
$sa_stmt->execute();
$sa_user = $sa_stmt->get_result()->fetch_assoc();
$sa_stmt->close();

$sa_name     = trim(($sa_user['firstname'] ?? '') . ' ' . ($sa_user['lastname'] ?? '')) ?: htmlspecialchars($_SESSION['username'] ?? 'Super Admin');
$sa_username = $sa_user['username'] ?? $_SESSION['username'] ?? 'superadmin';
$sa_email    = $sa_user['email'] ?? 'superadmin@yesparency.gov';
$sa_role     = strtoupper($sa_user['role'] ?? 'SUPERADMIN');
$sa_since    = !empty($sa_user['created_at']) ? date('M Y', strtotime($sa_user['created_at'])) : '2026';
$sa_pic      = !empty($sa_user['profile_picture_url']) ? '../' . ltrim($sa_user['profile_picture_url'], '/') : (!empty($_SESSION['profile_picture_url']) ? '../' . ltrim($_SESSION['profile_picture_url'], '/') : '');

// ── User stats ──────────────────────────────────────────────────────────────
$total_users   = (int)($conn->query("SELECT COUNT(*) FROM users WHERE role != 'superadmin'")->fetch_row()[0] ?? 0);
$total_admins  = (int)($conn->query("SELECT COUNT(*) FROM users WHERE role = 'admin'")->fetch_row()[0] ?? 0);
$total_bidders = (int)($conn->query("SELECT COUNT(*) FROM users WHERE role = 'bidder'")->fetch_row()[0] ?? 0);
$total_normal  = (int)($conn->query("SELECT COUNT(*) FROM users WHERE role = 'user'")->fetch_row()[0] ?? 0);
...
<head>
    ...
    <style>
        ...
        .compact-stat-metrics.three-col {
            grid-template-columns: repeat(3, 1fr);
        }

        .compact-stat-metrics.two-col {
        ...
        /* ── Side-by-Side: Procurement & Bids Stats (60% Column) ── */
        .dash-stats-side-by-side {
            display: grid;
        ...
        .side-event-item:hover .side-event-title {
            color: #1f7a3d;
        }
...
<div class="dash-content">
    ...
    <div class="dash-layout-60-40">
        ...
        <!-- ════════════════════════════════════════════════════════
             LEFT COLUMN (60%)
             1. Greetings Card
             2. User Statistics (Above Procurement & Bids)
             3. Procurement & Bids Stats (Side-by-Side)
        ...
        <div class="dash-col-60">
            ...
            <div class="dash-greeting-card">
                <div class="greeting-header">
                    <div class="greeting-title">Welcome back, <?= htmlspecialchars($sa_name) ?> </div>
                    <span class="greeting-badge">
                        <i class="bi bi-shield-lock-fill"></i> SUPER ADMINISTRATOR
                    </span>
                </div>
                <div class="greeting-sub">
                    Executive Governance &amp; Administration Portal — Oversee all municipal accounts, system security, procurements, and BAC activities.
                </div>
                <div class="greeting-pills">
                    <div class="greeting-pill">
                        <i class="bi bi-people-fill"></i> <strong><?= $total_users ?></strong> Registered Users
                    </div>
                    <div class="greeting-pill">
            ...
            <!-- 2. User Demographics / User Statistics (Above Procurement & Bids) -->
            <div class="compact-stat-card">
                <div class="compact-stat-top">
                    <div class="compact-stat-title">
                        <i class="bi bi-people-fill" style="color:#06251b;"></i> User Demographics &amp; Accounts
                    </div>
                    <a href="account-management.php" class="compact-stat-link">Manage Users <i class="bi bi-arrow-right"></i></a>
                </div>
                <div class="compact-stat-body">
                    <div class="compact-donut-wrap" style="background:conic-gradient(#219653 0% <?= pct($total_admins, max($total_users,1)) ?>%, #F0B92E 0% <?= pct($total_admins+$total_bidders, max($total_users,1)) ?>%, #2F6FED 0% 100%);">
                        <div class="compact-donut-inner">
                            <span class="compact-donut-num"><?= $total_users ?></span>
                            <span style="font-size:8px; color:#88968d; font-weight:700;">USERS</span>
                        </div>
                    </div>
                    <div class="compact-stat-metrics three-col">
                        <div class="compact-metric-item">
                            <div class="compact-metric-num" style="color:#219653;"><?= $total_admins ?></div>
                            <div class="compact-metric-lbl">Administrators</div>
                        </div>
                        <div class="compact-metric-item">
                            <div class="compact-metric-num" style="color:#C99A1D;"><?= $total_bidders ?></div>
                            <div class="compact-metric-lbl">Bidders</div>
                        </div>
                        <div class="compact-metric-item">
                            <div class="compact-metric-num" style="color:#2F6FED;"><?= $total_normal ?></div>
                            <div class="compact-metric-lbl">Citizens / Public</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- 3. Procurement & Bids Stats (Side by Side) -->
            <div class="dash-stats-side-by-side">

                <!-- Procurement Stats (Reduced Height) -->
                <div class="compact-stat-card" style="margin-bottom:0;">
                    <div class="compact-stat-top">
                        <div class="compact-stat-title">
                            <i class="bi bi-pie-chart-fill" style="color:#1f7a3d;"></i> Procurements
                        </div>
                        <a href="procurement.php" class="compact-stat-link">Manage <i class="bi bi-arrow-right"></i></a>
                    </div>
                    <div class="compact-stat-body">
                        ...
                        <div class="compact-stat-metrics two-col">
                            <div class="compact-metric-item">
                ...
                <!-- Bid Stats (Reduced Height) -->
                <div class="compact-stat-card" style="margin-bottom:0;">
                    <div class="compact-stat-top">
                        <div class="compact-stat-title">
                            <i class="bi bi-inbox-fill" style="color:#2F6FED;"></i> Bid Submissions
                        </div>
                        <a href="bid_submissions.php" class="compact-stat-link">View All <i class="bi bi-arrow-right"></i></a>
                    </div>
                    <div class="compact-stat-body">
                        ...
                        <div class="compact-stat-metrics two-col">
                            <div class="compact-metric-item">
                                ...
                                <div class="compact-metric-lbl">Pending</div>
                            </div>
                            <div class="compact-metric-item">
            ...
            <!-- 5. System Announcements & Broadcasts -->
            <div class="sub-card">
                <div class="sub-card-head">
                    <div class="sub-card-title">
                            ...
                            <div class="side-notif-body">
                                ...
                                <div style="display:flex; align-items:center; justify-content:space-between; margin-top:4px; gap:8px;">
                                    <div class="side-notif-meta">
                                        ...
                                        <span>By: <?= htmlspecialchars(trim(($nt['firstname'] ?? '') . ' ' . ($nt['lastname'] ?? '')) ?: 'Superadmin') ?></span>
                                    </div>
            ...
            <div class="profile-card">
                <div class="profile-top">
                    <div class="profile-avatar-box">
                        <?php if (!empty($sa_pic)): ?>
                            <img src="<?= htmlspecialchars($sa_pic) ?>" alt="<?= htmlspecialchars($sa_name) ?>">
                        <?php else: ?>
                            <?= strtoupper(substr($sa_name, 0, 1)) ?>
                        <?php endif; ?>
                    </div>
                    <div class="profile-info">
                        <div class="profile-biz-name"><?= htmlspecialchars($sa_name) ?></div>
                        <div class="profile-rep-name">
                            <i class="bi bi-at"></i> <?= htmlspecialchars($sa_username) ?>
                        </div>
                        <span class="profile-status-badge">
                            <i class="bi bi-shield-lock-fill"></i> <?= $sa_role ?>
                        </span>
                    </div>
                </div>
                ...
                <div class="profile-details-grid">
                    <div class="profile-detail-item">
                        <div class="profile-detail-lbl">Access Level</div>
                        <div class="profile-detail-val">Root Authority</div>
                    </div>
                    <div class="profile-detail-item">
                        <div class="profile-detail-lbl">Member Since</div>
                        <div class="profile-detail-val"><?= $sa_since ?></div>
                    </div>
                    <div class="profile-detail-item" style="grid-column: span 2;">
                        <div class="profile-detail-lbl">Superadmin Email</div>
                        <div class="profile-detail-val" title="<?= htmlspecialchars($sa_email) ?>"><?= htmlspecialchars($sa_email) ?></div>
                    </div>
                </div>
            </div>
    ...
    <div class="sad-actions">
        ...
        <a href="user-role-management.php" class="sad-action-card">
            <div class="sad-action-icon" style="background:#f3e5f5; color:#7b1fa2;"><i class="bi bi-person-gear"></i></div>
        ...
        <a href="announcements.php" class="sad-action-card">
            <div class="sad-action-icon" style="background:#fce4ec; color:#c2185b;"><i class="bi bi-megaphone"></i></div>
            <span>Announcements</span>
        </a>
        <a href="../logout.php" class="sad-action-card">
            <div class="sad-action-icon" style="background:#ffebee; color:#c62828;"><i class="bi bi-box-arrow-left"></i></div>
            <span>Logout</span>
        </a>
    </div>
