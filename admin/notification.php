...
");
$stat_total_stmt->bind_param("sii", $user_role, $user_id, $user_id);
...
");
$stat_unread_stmt->bind_param("isii", $user_id, $user_role, $user_id, $user_id);
...
");
$stat_broadcast_res->bind_param("i", $user_id);
...
$stat_direct_stmt = $conn->prepare("
    ...
    WHERE ((sn.target_type = 'role' AND sn.target_role = ?)
      ...
      AND sn.created_at >= (SELECT created_at FROM users WHERE user_id = ?)
");
$stat_direct_stmt->bind_param("sii", $user_role, $user_id, $user_id);
...
<head>
    ...
    <link rel="stylesheet" href="../css/pages/admin-notification.css">
</head>
...
<div class="dash-content">
    ...
    <div class="ap2-stats ap2-stats-4 mb-20">
        ...
        <div class="ap2-stat">
            <div class="ap2-ring" style="--ring-color:#06251b; --pct:100%;">
                <div class="ap2-ring-inner"><i class="bi bi-bell-fill clr-dark"></i></div>
            </div>
        ...
        <div class="ap2-stat <?= $stat_unread > 0 ? 'bsv-stat-warn' : '' ?>">
            <div class="ap2-ring" style="--ring-color:<?= $stat_unread > 0 ? '#e67e22' : '#8B958E' ?>; --pct:<?= $stat_total > 0 ? round($stat_unread / max($stat_total,1) * 100) : 0 ?>%;">
                <div class="ap2-ring-inner"><i class="bi bi-bell-slash-fill <?= $stat_unread > 0 ? 'clr-unread' : 'clr-unread-off' ?>"></i></div>
            </div>
            <div class="ap2-stat-text">
                <div class="ap2-stat-num <?= $stat_unread > 0 ? 'ap2-stat-num--unread' : '' ?>"><?= number_format($stat_unread) ?></div>
        ...
        <div class="ap2-stat">
            <div class="ap2-ring" style="--ring-color:#219653; --pct:<?= $stat_total > 0 ? round($stat_broadcast / max($stat_total,1) * 100) : 0 ?>%;">
                <div class="ap2-ring-inner"><i class="bi bi-globe clr-broadcast"></i></div>
            </div>
            <div class="ap2-stat-text">
        ...
        <div class="ap2-stat">
            <div class="ap2-ring" style="--ring-color:#2F6FED; --pct:<?= $stat_total > 0 ? round($stat_direct / max($stat_total,1) * 100) : 0 ?>%;">
                <div class="ap2-ring-inner"><i class="bi bi-person-badge clr-direct"></i></div>
            </div>
            <div class="ap2-stat-text">
    ...
    <div class="proc-table-panel mb-24">
        ...
        <div class="filter-bar">
            <form method="GET" action="notification.php" id="notifForm" class="at-filter-form">
        ...
        <div class="anc-empty-state">
            <i class="bi bi-bell-slash anc-empty-icon"></i>
            <div class="anc-empty-text">No notifications found<?= $search ? ' for "'.htmlspecialchars($search).'"' : '' ?></div>
            <?php if ($search !== '' || $filter_type !== 'all'): ?>
                <a href="notification.php" class="vp-back-link anc-empty-reset">Reset Filters</a>
            <?php endif; ?>
        </div>
        <?php else: ?>
            ...
            <table class="proc-table">
                <thead>
                    <tr>
                        ...
                        <th class="at-actions-th">Actions</th>
                    </tr>
                </thead>
                <tbody>
                <?php while ($nt = $notifs_result->fetch_assoc()):
                    ...
                    if ($tt === 'role') {
                        $r = $nt['target_role'];
                        if ($r === 'user')   { $pillMod='user';   $badgeTxt='Users';    $badgeIcn='person'; }
                        elseif ($r === 'bidder') { $pillMod='bidder'; $badgeTxt='Bidders';  $badgeIcn='person-badge'; }
                        elseif ($r === 'admin')  { $pillMod='admin';  $badgeTxt='Admins';   $badgeIcn='shield-check'; }
                        else                 { $pillMod='other'; $badgeTxt=strtoupper($r); $badgeIcn='people'; }
                    } elseif ($tt === 'user') { $pillMod='direct'; $badgeTxt='Direct'; $badgeIcn='person'; }
                ...
                <tr id="notif-row-<?= $nt['notification_id'] ?>" class="<?= !$isRead ? 'notif-row-unread' : '' ?>">
                    ...
                    <td class="col-mode">
                        <span class="proc-status-pill notif-target-pill notif-target-pill--<?= $pillMod ?>">
                    ...
                    <td class="proc-deadline-cell col-abc nowrap">
                        <?= date('M j, Y', strtotime($nt['created_at'])) ?><br>
                        <span class="clr-idle"><?= timeAgo($nt['created_at']) ?></span>
                    </td>
                    <td class="text-right">
